...
var DB = {
  ...
  passwd: {
    summary: "change a user's password (and password-aging settings)",
    takesValue: ["n", "x", "w", "i"],
    flags: {
      l: "lock the account's password (prefix it with !)",
      u: "unlock a locked password",
      d: "delete the password, leaving the account passwordless",
      e: "expire the password now, forcing a change at next login",
      S: "show password status (locked/usable, last change)",
      a: "with -S, report status for all accounts",
      n: "minimum days between password changes",
      x: "maximum days a password stays valid",
      w: "days of warning before a password expires",
      i: "days after expiry until the account is disabled"
    }
  },
  useradd: {
  ...
  usermod: {
    ...
    flags: {
      ...
      G: "set supplementary groups (replaces the list unless -a)",
      a: "append to groups instead of replacing (use with -G)",
      s: "change the login shell",
      d: "change the home directory path",
      m: "move the contents of the old home to the new one (with -d)",
      L: "lock the account's password",
      U: "unlock the account's password",
      u: "change the numeric user ID",
      g: "change the primary group",
      c: "change the comment / full-name field",
      e: "change the account expiration date",
      "--append": "append to groups instead of replacing (use with -G)",
      "--groups": "set supplementary groups",
      "--login": "change the login name",
      "--shell": "change the login shell",
      "--home": "change the home directory path"
    }
  },
  groupadd: {
    ...
    takesValue: ["g", "--gid"],
    flags: {
      g: "specific numeric group ID",
      r: "create a system group",
      f: "exit successfully if the group already exists",
      o: "allow creating a group with a duplicate (non-unique) GID",
      "--system": "create a system group",
      "--gid": "specific numeric group ID"
    }
  },
  lscpu: {
    summary: "display information about the CPU architecture",
    flags: {
      e: "extended, human-readable per-CPU table",
      p: "parseable (CSV) per-CPU output",
      a: "print both online and offline CPUs",
      b: "print only online CPUs",
      c: "print only offline CPUs",
      x: "show CPU masks in hexadecimal",
      J: "output in JSON",
      "--extended": "extended, human-readable per-CPU table",
      "--parse": "parseable (CSV) per-CPU output",
      "--json": "output in JSON"
    }
  },
  lspci: {
    ...
    takesValue: ["s", "d"],
    flags: {
      v: "verbose \u2014 more detail per device (repeat for more)",
      n: "show numeric vendor/device IDs instead of names",
      k: "show the kernel driver and modules handling each device",
      m: "machine-readable output",
      t: "show devices as a bus tree",
      s: "select a device by [[[[domain:]bus:]slot][.func]]",
      d: "select devices by [vendor]:[device] ID",
      x: "dump the configuration space in hex"
    }
  },
  lsusb: {
    summary: "list USB buses and the devices connected to them",
    takesValue: ["s", "d"],
    flags: {
      v: "verbose \u2014 full descriptor details per device",
      t: "show devices as a hierarchical tree",
      s: "show only a device on the given [bus]:[devnum]",
      d: "show only devices with the given [vendor]:[product] ID"
    }
  },
  lsmod: {
    summary: "show the status of loaded kernel modules (reads /proc/modules)",
    flags: {}
  },
  modprobe: {
    summary: "add or remove modules from the Linux kernel",
    flags: {
      r: "remove (unload) the module instead of loading it",
      a: "insert all modules named on the command line",
      v: "verbose \u2014 print what is being done",
      n: "dry run \u2014 show actions without applying them",
      f: "force load/unload, ignoring version checks",
      q: "quiet \u2014 suppress error messages for missing modules",
      "--remove": "remove (unload) the module",
      "--dry-run": "show actions without applying them"
    }
  },
  shuf: {
    summary: "write a random permutation of its input lines",
    takesValue: ["n", "o", "i", "--head-count", "--output", "--input-range"],
    flags: {
      e: "treat each argument as an input line (echo mode)",
      n: "output at most this many lines",
      o: "write the result to this file instead of stdout",
      r: "allow lines to repeat in the output",
      i: "shuffle the integers in this LO-HI range",
      z: "line delimiter is NUL, not newline",
      "--head-count": "output at most this many lines",
      "--output": "write the result to this file",
      "--input-range": "shuffle the integers in this LO-HI range",
      "--repeat": "allow lines to repeat in the output"
    }
  },
  split: {
    summary: "split a file into fixed-size or fixed-count pieces",
    takesValue: [
      "l",
      "b",
      "n",
    ...
    flags: {
      l: "put this many lines in each output file",
      b: "put this many bytes in each output file (accepts K/M/G)",
      n: "split into this many equal chunks",
      d: "use numeric suffixes (00, 01, \u2026) instead of letters",
      a: "length of the generated suffixes",
      C: "at most this many bytes per file, but keep lines whole",
      x: "use hexadecimal suffixes",
      "--lines": "put this many lines in each output file",
      "--bytes": "put this many bytes in each output file",
      "--number": "split into this many equal chunks",
      "--suffix-length": "length of the generated suffixes",
      "--additional-suffix": "append this suffix to output file names"
    }
  },
  strings: {
    summary: "print the printable character sequences found in a file",
    takesValue: ["n", "t", "e", "--bytes", "--radix", "--encoding"],
    flags: {
      a: "scan the entire file, not just loadable sections",
      n: "minimum string length to report (default 4)",
      t: "print the offset of each string (o/x/d radix)",
      f: "print the file name before each string",
      e: "select the character encoding (s/b/l/B/L)",
      "--bytes": "minimum string length to report",
      "--radix": "print the offset of each string (o/x/d)",
      "--encoding": "select the character encoding"
    }
  },
  ldd: {
    summary: "print the shared libraries a program or library depends on",
    flags: {
      v: "verbose \u2014 print all information, including symbol versioning",
      u: "print unused direct dependencies",
      r: "perform relocations for both data and functions",
      d: "perform relocations for data objects"
    }
  },
  screen: {
    summary: "terminal multiplexer \u2014 detachable sessions that survive logout",
    takesValue: ["S", "p", "t"],
    flags: {
      S: "name the new session",
      r: "reattach to a detached session",
      R: "reattach if possible, otherwise start a new session",
      d: "detach a session (with -r, detach elsewhere then reattach)",
      D: "detach and log out the remote session, then reattach",
      x: "attach to a session that is not detached (multi-attach)",
      m: "force a new session even when one exists (with -d = detached)"
    }
  },
  zcat: {
    summary: "decompress gzip-compressed files and write the result to stdout",
    flags: {
      f: "force \u2014 read even from a terminal or non-gzip input",
      q: "suppress warning messages"
    }
  },
  sha512sum: {
    summary: "compute or verify SHA-512 checksums",
    flags: {
      c: "read checksums from the file(s) and verify them",
      b: "read files in binary mode",
      t: "read files in text mode",
      q: "with -c, print nothing; signal failure via exit status",
      w: "with -c, warn about improperly formatted checksum lines",
      "--check": "read checksums from the file(s) and verify them",
      "--quiet": "with -c, print nothing; signal failure via exit status"
    }
  },
  iptables: {
    summary: "administer the Linux kernel IPv4 packet-filter rules (netfilter firewall)",
    takesValue: [
      "A",
      "I",
      "D",
    ...
    flags: {
      A: "append a rule to the end of the given chain",
      I: "insert a rule at a position in a chain (default: the top)",
      D: "delete a matching rule from a chain",
      L: "list all rules in a chain",
      F: "flush \u2014 delete every rule in a chain",
      N: "create a new user-defined chain",
      X: "delete an empty user-defined chain",
      P: "set the default policy (ACCEPT/DROP) for a built-in chain",
      p: "match a protocol (tcp, udp, icmp, ...)",
      s: "match a source address or subnet",
      d: "match a destination address or subnet",
      j: "jump to a target (ACCEPT, DROP, REJECT, ...) on a match",
      i: "match the input network interface",
      o: "match the output network interface",
      m: "load an extension match module (e.g. state, conntrack)",
      n: "numeric output \u2014 don't resolve host names or ports",
      v: "verbose output",
      t: "operate on the given table (filter, nat, mangle, raw)",
      "--dport": "match the destination port",
      "--sport": "match the source port",
      "--state": "match connection state (NEW, ESTABLISHED, RELATED)",
      "--line-numbers": "show rule line numbers when listing"
    }
  },
  ufw: {
    summary: "Uncomplicated Firewall \u2014 a friendly front-end for the iptables firewall",
    subcommands: {
      enable: "turn the firewall on (and at boot)",
      disable: "turn the firewall off",
      allow: "add a rule permitting traffic",
      deny: "add a rule silently dropping traffic",
      reject: "add a rule rejecting traffic (sends an error back)",
      delete: "remove a previously added rule",
      status: "show the firewall state and active rules",
      reload: "reload the firewall rules",
      reset: "disable and reset to factory defaults",
      default: "set the default policy for incoming/outgoing traffic",
      limit: "allow but rate-limit connections (brute-force protection)",
      logging: "set the logging level"
    },
    flags: {
      "--dry-run": "show the resulting rule set without applying it",
      "--force": "apply without the interactive confirmation prompt"
    }
  },
  "firewall-cmd": {
    summary: "manage firewalld \u2014 the dynamic firewall daemon on RHEL/Fedora systems",
    takesValue: [
      "--zone",
      "--add-port",
      "--remove-port",
    ...
    flags: {
      "--zone": "operate on the given zone",
      "--add-port": "open a port (e.g. 80/tcp) in the zone",
      "--remove-port": "close a previously opened port",
      "--add-service": "allow a named service (e.g. http, ssh)",
      "--remove-service": "disallow a named service",
      "--add-rich-rule": "add a fine-grained 'rich' rule",
      "--list-all": "show every setting for the zone",
      "--permanent": "make the change persist across reloads/reboots",
      "--reload": "reload rules from the permanent configuration",
      "--state": "report whether firewalld is running",
      "--get-zones": "list the available zones",
      "--get-active-zones": "list zones that have interfaces bound to them"
    }
  },
  strace: {
    summary: "trace the system calls and signals a process makes",
    takesValue: ["e", "p", "o", "s"],
    flags: {
      f: "also trace child processes as they are forked",
      e: "filter which system calls or events to trace",
      p: "attach to and trace an already-running PID",
      o: "write the trace to a file instead of standard error",
      c: "count time, calls and errors per syscall, print a summary",
      t: "prefix each line with the time of day",
      T: "show the time spent inside each system call",
      s: "set the maximum length of printed strings (default 32)",
      v: "verbose \u2014 print unabbreviated structs, env, etc.",
      y: "print paths for file-descriptor arguments"
    }
  },
  ltrace: {
    summary: "trace the library (and optionally system) calls a program makes",
    takesValue: ["e", "p", "o", "s"],
    flags: {
      ...
      p: "attach to and trace an already-running PID",
      o: "write the trace to a file instead of standard error",
      c: "summarize call counts and time on exit",
      S: "also display system calls",
      s: "set the maximum length of printed strings"
    }
  },
  "systemd-analyze": {
    summary: "inspect and profile systemd boot performance and unit state",
    subcommands: {
      time: "show how long firmware, loader, kernel and userspace took to boot",
      blame: "list units ordered by how long they took to initialise",
      "critical-chain": "show the time-critical chain of units at boot",
      plot: "write an SVG timeline of the boot to standard output",
      dump: "print the internal server state",
      verify: "check unit files for errors",
      security: "score the sandboxing/exposure of a service unit"
    },
    flags: {
      "--user": "operate on the per-user manager, not the system manager",
      "--no-pager": "do not pipe output into a pager"
    }
  },
  hostnamectl: {
    summary: "query and change the system hostname and related metadata",
    subcommands: {
      status: "show the current hostname and machine information",
      "set-hostname": "set the system hostname",
      "set-icon-name": "set the icon name shown in graphical tools",
      "set-chassis": "set the chassis type (laptop, server, vm, ...)",
      "set-location": "set a free-form location string"
    },
    flags: {
      "--static": "operate on the static (persistent) hostname",
      "--pretty": "operate on the pretty (free-form) hostname",
      "--transient": "operate on the transient (runtime) hostname",
      "--no-ask-password": "do not prompt for authentication"
    }
  },
  timedatectl: {
    summary: "query and change the system clock, time zone and NTP settings",
    subcommands: {
      status: "show the current time, time zone and NTP state",
      "set-time": "set the system clock to the given date/time",
      "set-timezone": "set the system time zone",
      "list-timezones": "list the known time-zone names",
      "set-ntp": "enable or disable network time synchronisation",
      "set-local-rtc": "store the hardware clock in local time (vs UTC)",
      "timesync-status": "show details of the running time-sync service"
    },
    flags: {
      "--no-pager": "do not pipe output into a pager",
      "--adjust-system-clock": "with set-local-rtc, also update the system clock"
    }
  },
  nice: {
    summary: "run a program with a modified scheduling priority (niceness)",
    takesValue: ["n", "--adjustment"],
    flags: {
      n: "set the niceness (-20 = highest priority .. 19 = lowest)",
      "--adjustment": "set the niceness increment"
    }
  },
  renice: {
    summary: "change the scheduling priority of already-running processes",
    takesValue: ["n"],
    flags: {
      n: "the new absolute niceness value",
      p: "interpret the following arguments as process IDs",
      g: "interpret the following arguments as process group IDs",
      u: "interpret the following arguments as user names"
    }
  },
  taskset: {
    summary: "set or read a process's CPU affinity (which cores it may use)",
    takesValue: ["c"],
    flags: {
      p: "operate on an existing PID instead of launching a command",
      c: "give the CPU list as numbers, e.g. 0,2,5-7 (not a hex mask)",
      a: "with -p, apply to all threads of the process"
    }
  },
  at: {
    summary: "schedule a command to run once at a specified later time",
    takesValue: ["f", "q"],
    flags: {
      f: "read the job from a file instead of standard input",
      m: "mail the user on completion, even with no output",
      l: "list the pending jobs (same as atq)",
      d: "delete a job by its number (same as atrm)",
      c: "print the named jobs to standard output",
      q: "place the job in the named queue"
    }
  },
  fold: {
    ...
    takesValue: ["w", "--width"],
    flags: {
      w: "set the maximum line width (default 80 columns)",
      s: "break at spaces so words are not split",
      b: "count bytes rather than display columns",
      "--width": "set the maximum line width",
      "--spaces": "break at spaces so words are not split",
      "--bytes": "count bytes rather than display columns"
    }
  },
  join: {
    summary: "join the lines of two files that share a common field",
    takesValue: ["t", "a", "v", "e", "o"],
    flags: {
      "1": "join on this field number from file 1",
      "2": "join on this field number from file 2",
      t: "use this character as the input/output field separator",
      a: "also print unpairable lines from file 1 or 2 (-a 1 / -a 2)",
      v: "print ONLY the unpairable lines from the given file",
      e: "replace empty output fields with this string",
      o: "control the output format (a list of fields)",
      i: "ignore case when comparing the join field"
    }
  },
  chattr: {
    summary: "change ext2/3/4 file attributes, e.g. +i makes a file immutable",
    flags: {
      R: "recurse into directories",
      V: "be verbose and print the program version",
      f: "suppress most error messages"
    }
  },
  getfacl: {
    summary: "display the Access Control Lists (ACLs) of files and directories",
    flags: {
      R: "recurse into subdirectories",
      a: "display only the file access control list",
      d: "display only the default access control list",
      t: "use the tabular output format",
      n: "show numeric user/group IDs instead of names",
      "--omit-header": "do not print the comment header (file/owner/group)"
    }
  }
};
var GENERIC_FLAGS = {
...
var EXAMPLES = {
  ...
  passwd: ["passwd", "passwd alice", "passwd -S bob", "passwd -l svc"],
  useradd: ["useradd -m -s /bin/bash alice", "useradd -r -s /usr/sbin/nologin svc", "useradd -m -G sudo,docker bob"],
  usermod: ["usermod -aG docker alice", "usermod -s /bin/zsh bob", "usermod -L olduser"],
  groupadd: ["groupadd docker", "groupadd -g 1005 staff", "groupadd -r sysgrp"],
  lscpu: ["lscpu", "lscpu -e", "lscpu --json"],
  lspci: ["lspci", "lspci -v", "lspci -nnk"],
  lsusb: ["lsusb", "lsusb -t", "lsusb -v"],
  lsmod: ["lsmod", "lsmod | grep nvidia"],
  modprobe: ["modprobe nvidia", "modprobe -r nvidia", "modprobe -n -v e1000"],
  shuf: ["shuf -n 1 file.txt", "shuf -i 1-100 -n 5", "shuf -e apple banana cherry"],
  split: ["split -l 1000 big.txt part_", "split -b 10M file.iso chunk_", "split -n 4 -d data seg_"],
  strings: ["strings binary", "strings -n 8 a.out", "strings -t x firmware.bin"],
  ldd: ["ldd /bin/ls", "ldd -v ./a.out"],
  screen: ["screen -S work", "screen -r", "screen -dm long-job.sh"],
  zcat: ["zcat access.log.gz", "zcat -f file", "zcat data.gz | grep error"],
  sha512sum: ["sha512sum file.iso", "sha512sum -c SHA512SUMS", "sha512sum *.tar.gz"],
  iptables: ["iptables -A INPUT -p tcp --dport 22 -j ACCEPT", "iptables -L -n -v", "iptables -t nat -A POSTROUTING -o eth0 -j MASQUERADE"],
  ufw: ["ufw allow 22/tcp", "ufw enable", "ufw status verbose"],
  "firewall-cmd": ["firewall-cmd --zone=public --add-port=80/tcp --permanent", "firewall-cmd --reload", "firewall-cmd --list-all"],
  strace: ["strace -f -e trace=open,read -o trace.log ./prog", "strace -p 1234 -c"],
  ltrace: ["ltrace -f -e malloc+free ./prog", "ltrace -p 1234 -c"],
  "systemd-analyze": ["systemd-analyze blame", "systemd-analyze critical-chain", "systemd-analyze security sshd"],
  hostnamectl: ["hostnamectl status", "hostnamectl set-hostname web01"],
  timedatectl: ["timedatectl set-timezone Europe/Paris", "timedatectl status", "timedatectl set-ntp true"],
  nice: ["nice -n 19 tar czf backup.tgz .", "nice -n 10 ./batch-job"],
  renice: ["renice -n 5 -p 1234", "renice 10 -u alice"],
  taskset: ["taskset -c 0,1 ./prog", "taskset -pc 0-3 1234"],
  at: ["at -f script.sh 09:00", "at -l"],
  fold: ["fold -w 72 -s letter.txt", "fold -w 40 notes.txt"],
  join: ["join -t , -a 1 users.csv orders.csv", "join file1 file2"],
  chattr: ["chattr -R +i logs/", "chattr -V +a app.log"],
  getfacl: ["getfacl file.txt", "getfacl -R /srv/www"]
};
